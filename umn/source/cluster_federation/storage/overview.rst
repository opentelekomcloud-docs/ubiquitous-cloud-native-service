...
-  UCS can automatically create EVS, OBS, and SFS volumes and mount them to the container path of a T Cloud cluster.
...
-  For a non-T Cloud cluster, when PVCs are used to mount cloud storage volumes, the cluster provider must support storage classes. For details, see `Storage Classes <https://kubernetes.io/docs/concepts/storage/storage-classes/>`__.
